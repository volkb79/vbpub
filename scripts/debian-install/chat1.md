...
1. **I/O efficiency**: 32 × 4KB = 128KB per batch
...
2. **CPU cache efficiency**: 32 pages fit well in L2/L3 cache
   - Kernel can process page descriptors efficiently
   - Reduces TLB pressure during reclaim

3. **Latency balance**: Small enough to avoid long pauses
...
1. **Disk seek cost**: HDD seeks are expensive (~10ms)
   - Reading 1 page: seek(10ms) + read(0.1ms) = 10.1ms
   - Reading 8 pages: seek(10ms) + read(0.8ms) = 10.8ms
   - 8× more data for 7% more time!

2. **Swap slot locality**: Pages swapped out together often accessed together
...
3. **Temporal locality**: If page N is accessed, pages N-1, N+1 likely needed soon
...
Configuration:
  ZSWAP cache:   3GB in RAM (lz4/zstd + zsmalloc)
  Disk swap:     8 partitions, 8GB total
  Swappiness:    80 (aggressive)
  Page cluster:  0 (no readahead)

Next Steps:
  1. Reboot:        reboot
  2. Verify:        check-zswap
  3. Monitor:       monitor-zswap
