...
setup_zswap() {
    ...
    if [ -d /sys/module/zswap ]; then
        ...
        # Set zpool (allocator) - prefer zbud for reliability with ZSWAP
        log_info "Setting ZSWAP zpool: $ZSWAP_ZPOOL"
        if echo "$ZSWAP_ZPOOL" > /sys/module/zswap/parameters/zpool 2>/dev/null; then
            log_info "✓ Zpool set to $ZSWAP_ZPOOL"
        else
            log_warn "Failed to set zpool to $ZSWAP_ZPOOL"
            # Fallback to zbud (most compatible)
            if [ "$ZSWAP_ZPOOL" != "zbud" ]; then
                if echo "zbud" > /sys/module/zswap/parameters/zpool 2>/dev/null; then
                    log_info "✓ Fallback: using zbud zpool (most compatible)"
                else
                    log_warn "Using system default zpool"
                fi
            fi
        fi
        
        # Enable shrinker (kernel 6.8+) for automatic cache pressure relief
        if [ -f /sys/module/zswap/parameters/shrinker_enabled ]; then
            if echo "Y" > /sys/module/zswap/parameters/shrinker_enabled 2>/dev/null; then
                log_info "✓ Enabled ZSWAP shrinker for automatic cache management"
            else
                log_debug "Could not enable shrinker (may require newer kernel)"
            fi
        else
            log_debug "ZSWAP shrinker not available (kernel < 6.8)"
        fi
    ...
    if ! grep -q "zswap.enabled" /etc/default/grub; then
        log_info "Adding ZSWAP to GRUB config with shrinker enabled"
        sed -i 's/GRUB_CMDLINE_LINUX_DEFAULT="/GRUB_CMDLINE_LINUX_DEFAULT="zswap.enabled=1 zswap.compressor='$ZSWAP_COMPRESSOR' zswap.max_pool_percent='$ZSWAP_POOL_PERCENT' zswap.shrinker_enabled=1 /' /etc/default/grub
        update-grub || log_warn "Failed to update GRUB"
    fi
    
    # Configure Transparent Huge Pages (THP) per chat-merged.md recommendations
    log_info "Configuring Transparent Huge Pages (THP)"
    
    # Set THP to madvise mode (not always)
    # Prevents memory bloat, inefficient swapping, and khugepaged latency spikes
    if [ -f /sys/kernel/mm/transparent_hugepage/enabled ]; then
        echo madvise > /sys/kernel/mm/transparent_hugepage/enabled 2>/dev/null || true
        log_info "Set THP to madvise mode (applications must opt-in)"
    fi
    
    if [ -f /sys/kernel/mm/transparent_hugepage/defrag ]; then
        echo never > /sys/kernel/mm/transparent_hugepage/defrag 2>/dev/null || true
        log_info "Disabled THP defragmentation (prevents latency spikes)"
    fi
    
    # Create THP configuration systemd service for persistence
    log_info "Creating THP systemd service for persistence"
    
    cat > /etc/systemd/system/thp-config.service <<'THPEOF'
[Unit]
Description=Configure Transparent Huge Pages
Documentation=https://github.com/volkb79/vbpub/blob/main/scripts/debian-install/chat-merged.md
...
THPEOF
    ...
    log_info "THP systemd service created and enabled"
...
configure_kernel_params() {
    ...
    # Calculate optimal values based on SWAP_RAM_SOLUTION
    local vfs_cache_pressure=100
    local watermark_scale=10
    local dirty_ratio=20
    local dirty_bg_ratio=10
    
    if [ "$SWAP_RAM_SOLUTION" = "zswap" ]; then
        # ZSWAP-optimized values per chat-merged.md
        swappiness=80  # More aggressive for ZSWAP (cheap swapping)
        vfs_cache_pressure=50  # Keep metadata cached
        watermark_scale=125  # Earlier reclaim trigger
        dirty_ratio=15
        dirty_bg_ratio=5
        log_info "Using ZSWAP-optimized kernel parameters"
    fi
...
# Overcommit settings (conservative)
